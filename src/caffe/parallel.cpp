...
#ifdef USE_NCCL
template<typename Dtype>
void P2PSync<Dtype>::setNCCLComm(ncclComm_t comm, int param_id) {
  this->nccl_comms_[param_id] = comm;
}

template<typename Dtype>
ncclComm_t P2PSync<Dtype>::getNCCLComm(int param_id) {
...
template<typename Dtype>
cudaStream_t P2PSync<Dtype>::getCommStream(int param_id) {
...
void P2PSync<Dtype>::Run(const vector<int>& gpus) {
  ...
  bar = new boost::barrier(gpus.size());
  ...
  size_t num_params = solver_->net()->learnable_params().size();
